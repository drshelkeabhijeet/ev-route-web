...
import { Zap, Mail, Lock, Leaf } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (errorMessage.toLowerCase().includes('email not confirmed') || errorMessage.includes('email_not_confirmed')) {
        toast.error('Please check your email and click the verification link.')
      } else if (errorMessage.toLowerCase().includes('too many requests') || errorMessage.includes('too_many_requests')) {
        toast.error('Too many login attempts. Please try again later.')
      } else if (errorMessage.toLowerCase().includes('supabase is not configured')) {
        toast.error('Authentication is not configured. Please contact support.')
      } else {
        console.log('Showing fallback toast') // Debug log
      ...
      <Card className="w-full max-w-md shadow-2xl border-green-100 dark:border-green-900/50">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-balance">
            Your partner in sustainable travel. Sign in to continue.
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="flex flex-col gap-4">
          <div className="text-center text-sm">
            ...
            <Link href="/signup" className="underline text-green-600 font-medium">
              Sign up
            </Link>
          </div>
